...
<CurrentVersion kind="native-thingd" :label="false" /> (`>=0.87.0, <0.88.0`). It supplies objects, events, search, links, durable
queues, encryption-aware persistence, and a public replication contract.
Thingd 0.87 (ThingDB 0.87.0) continues the ThingDB backend introduced in 0.86, with bounded storage caches,
...
off runs through the queue contract. Native Thingd 0.87.0 (and 0.86.0) supports
...
### Native adapter Thingd 0.87.0 persistent asynchronous search

Thingd 0.87.0 (continuing 0.86.0) provides coalesced asynchronous Tantivy indexing while the
...
Thingd 0.87.0 also provides bounded large-journal recovery for low-memory hosts (introduced in 0.86.0):
...
### Upgrading native storage to Thingd 0.87.0

Thingd 0.87.0 uses its current persistent storage contract (unchanged from 0.86.0) and no longer
