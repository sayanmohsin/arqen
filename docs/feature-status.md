...
| Native durable Thingd       | Available / early     | Optional `thingd-native` feature, compatible with Thingd <CurrentVersion kind="native-thingd" :label="false" />. Recovery, backups, resource sizing, and workload testing remain deployment work. |
| Native maintenance          | Experimental          | Optional `thingd-maintenance` diagnostics, path validation, compaction, and bounded search-rebuild controls.                                                                                      |
| Native connectors           | Experimental          | Optional `thingd-connectors` access to Thingd file, spreadsheet, and SQL connector APIs; credentials and source policy remain application-owned.                                                  |
