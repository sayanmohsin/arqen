...
impl HttpThingdBackend {
    ...
    async fn get<R: for<'de> Deserialize<'de>>(&self, path: &str) -> Result<R, arqen_core::AppError> {
        ...
        let response = request.send().await.map_err(|e| {
            arqen_core::AppError::new(arqen_core::ErrorKind::External, format!("HTTP request failed: {}", e))
        })?;
        if !response.status().is_success() {
            let status = response.status().as_u16();
            let text = response.text().await.unwrap_or_default();
            return Err(arqen_core::AppError::new(
                arqen_core::ErrorKind::External,
                format!("HTTP error {}: {}", status, text),
            ));
        }
        response.json().await.map_err(|e| {
            arqen_core::AppError::new(arqen_core::ErrorKind::External, format!("Failed to parse response: {}", e))
        })
    }

    async fn post<T: Serialize, R: for<'de> Deserialize<'de>>(&self, path: &str, body: T) -> Result<R, arqen_core::AppError> {
        let url = format!("{}{}", self.base_url, path);
    ...
    async fn put<T: Serialize, R: for<'de> Deserialize<'de>>(&self, path: &str, body: T) -> Result<R, arqen_core::AppError> {
        let url = format!("{}{}", self.base_url, path);
    ...
    async fn delete<R: for<'de> Deserialize<'de>>(&self, path: &str) -> Result<R, arqen_core::AppError> {
        let url = format!("{}{}", self.base_url, path);
...
impl ThingdBackend for HttpThingdBackend {
    async fn get_object(&self, collection: &str, id: &str) -> Result<Option<ThingdObject>, arqen_core::AppError> {
    ...
    async fn put_object(&self, collection: &str, id: &str, data: serde_json::Value) -> Result<ThingdObject, arqen_core::AppError> {
        let path = format!("/collections/{}/objects/{}", collection, id);
    ...
    async fn query_objects(&self, collection: &str, filter: Option<ThingdFilter>) -> Result<Vec<ThingdObject>, arqen_core::AppError> {
    ...
    async fn batch_write(&self, operations: Vec<ThingdOperation>) -> Result<Vec<ThingdOperationResult>, arqen_core::AppError> {
        let path = "/batch";
        self.post(path, operations).await
    }

    async fn append_event(&self, stream: &str, event_type: &str, data: serde_json::Value) -> Result<ThingdEvent, arqen_core::AppError> {
        let path = format!("/streams/{}/events", stream);
        let body = serde_json::json!({ "event_type": event_type, "data": data });
        self.post(&path, body).await
    }

    async fn read_events(&self, stream: &str, from: Option<String>, limit: usize) -> Result<Vec<ThingdEvent>, arqen_core::AppError> {
    ...
    async fn push_job(&self, queue: &str, payload: serde_json::Value, max_retries: u32) -> Result<ThingdJob, arqen_core::AppError> {
        let path = format!("/queues/{}/jobs", queue);
        let body = serde_json::json!({ "payload": payload, "max_retries": max_retries });
        self.post(&path, body).await
    }

    async fn claim_job(&self, queue: &str, worker_id: &str, lease_seconds: u32) -> Result<Option<ThingdJob>, arqen_core::AppError> {
    ...
    async fn complete_job(&self, queue: &str, job_id: &str) -> Result<(), arqen_core::AppError> {
        ...
        self.post::<_, EmptyResponse>(&path, serde_json::json!({})).await?;
        Ok(())
    }
    ...
    async fn search(&self, query: &str, options: SearchOptions) -> Result<SearchResults, arqen_core::AppError> {
        let path = "/search";
        let body = serde_json::json!({ "query": query, "options": options });
        self.post(path, body).await
    }

    async fn create_link(&self, source_id: &str, target_id: &str, relation: &str) -> Result<ThingdLink, arqen_core::AppError> {
        let path = "/links";
        let body = serde_json::json!({ "source_id": source_id, "target_id": target_id, "relation": relation });
        self.post(path, body).await
    }

    async fn get_links(&self, source_id: &str, relation: Option<&str>) -> Result<Vec<ThingdLink>, arqen_core::AppError> {
